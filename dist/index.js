          ...
          for (const key of keys) {
            ...
            for (const item of items) {
              if (typeof item === "string" && item.trim()) {
                related.push(simplifySlug(stripWikilink(item)));
              }
            }
          }
          if (related.length > 0) {
            file.data.related = related;
          }
        }
      ];
    },
    htmlPlugins() {
      return [
        () => (_tree, file) => {
          ...
          const merged = Array.from(/* @__PURE__ */ new Set([...existing, ...related]));
          data.links = merged;
        }
      ];
    }
...
export { Related_default as Related, RelatedTransformer };
